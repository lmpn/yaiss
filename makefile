prepare:
	...
	cargo sqlx prepare --merged --database-url sqlite:backend/sql/test.db;
...
backend-coverage: backend-prepare
	DB_URL=sqlite:sql/test.db INI_CONFIGURATION=resources/dev.configuration.ini cargo llvm-cov --all-features --workspace --lcov --ignore-filename-regex "bin|services/images/ports/|web/mod.rs"  --output-path lcov.info
